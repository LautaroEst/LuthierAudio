...
def window_LPC_coef(signal,N=20):
    r = signal.xcorr(mode='full')
    ro = r[signal.L-1:signal.L+N] # np.size(ro) = N + 1
    b = ro[1:N+1]
...
def lpc_compress(sig, t_window, delta_t, N_lpc, Nbits=16):
    """
    ...
    sig_error = AudioSignal(Fs=sig_completa.Fs)
    zf = np.zeros(N_lpc)
    ...
    sig_error_cuant = AudioSignal(bins[ints], Fs=sig_error.Fs)
...
def ReconstruirSignal(LPC_matrix, sig_error,delta_t):
    ...
    sig_reconstruida = AudioSignal(Fs=sig_completa.Fs)
