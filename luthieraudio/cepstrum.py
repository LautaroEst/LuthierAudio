...
def get_ceps_coef(signal,N=20,Nfft=2**14,window='boxcar'):
    x_hat = np.real(np.fft.ifft(np.log(np.abs(signal.fft(Nfft))),Nfft))       
    w = get_window(window,2*N) # ventana para hacer el liftering
    x_lift = x_hat[:N] * w[-N:]
    return x_lift


def cepstrum_signal(sig=None, t_window=.01, delta_t=.01, N_ceps=20, Nfft=0, window_ceps='boxcar'):
    """
