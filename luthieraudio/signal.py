...
from .lpc import window_LPC_coef
from .cepstrum import get_ceps_coef
...
class AudioSignal:
    ...
    def from_wav(cls, filename):
        ...
        una instancia de AudioSignal() cuyo vector x siempre es de tipo np.float64 y tiene valores 
        entre -1 y 1. 
        """
    ...
    def glotic(cls, F0=125, Fs=44100, t_max=1.,mode='Delta Train'):
        """
        Crea una señal que representa la fuente glótica modelada 
    ...
    def xcorr(self,mode='full'):
        ...
        """
        return np.correlate(self.x,self.x,mode=mode)
    
        
    def get_lpc_coef(self, N=20):
        """
        Calcula los N coeficientes LPC de la señal.
        """
        a, G = window_LPC_coef(self,N)
        return a, G

    
    def get_cepstrum_coef(self,N=20,Nfft=2**14,window='boxcar'):
        """
            Calcula los N primeros coeficientes cepstrum de la señal.
        """
        x_lift = get_ceps_coef(self,N=20,Nfft=2**14,window='boxcar')
        return x_lift
    ...
    def copy(self):
        ...
        """
        sig_copy = AudioSignal(self.x.copy(), Fs=self.Fs, t0=self.t[0].copy())
        return sig_copy
    ...
    def append(self,sig1,where='end'):
        """
            Recibe sig1 que puede ser un ndarray o un AudioSignal y lo agrega a la señal
        """
        
        if isinstance(sig1, np.ndarray):
            x_append = sig1.copy()
        elif isinstance(sig1, AudioSignal):
            x_append = sig1.x.copy()
        else:
            print('La señal anexada tiene que ser un objeto AudioSignal o ndarray')
            return
...
# ---------------------------------------------------------------------
class fonema(AudioSignal):
    
    def __init__(self, x, Fs, t0, fon='a'):
        super().__init__(x, Fs, t0) # Esto copia los atributos x, Fs y t0 de AudioSignal a la clase fonema
        self.fon = fon
